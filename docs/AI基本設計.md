...
- リアルタイムLLM、Embedding、バッチLLM候補の比較
- SAFETYとMeaning抽出を分離する案・統合する案の比較
- Meaning Structureの候補スキーマとEmbedding対象の比較
- 検索件数、LLM投入件数、評価軸、重み、閾値の検証
- Railsのみの最終式とAI最終決定を含む方式比較
- JSON成功率、応答時間、コスト、再現性の計測
